...
foreach($resp as $elem)  {
   	 $kycID = $elem['kyc']['kycID']; 
		 $description = $elem['kyc']['description'];
		 $kycLevel = $elem['kyc']['kycLevel'];
		 $dailyTransactionLimit = $elem['kyc']['dailyTransactionLimit'];
		 $cumulativeBalanceLimit = $elem['kyc']['cumulativeBalanceLimit'];
		 
		 
		 if(is_numeric($kycLevel))
		 {
 echo json_encode(array("statusCode"=>200, "kycID"=>$kycID,"description"=>$description,"kycLevel"=>$kycLevel,"dailyTransactionLimit"=>$dailyTransactionLimit,"cumulativeBalanceLimit"=>$cumulativeBalanceLimit));  		 
		 }
}
