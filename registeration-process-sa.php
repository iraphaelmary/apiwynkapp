...
if(empty($phone))
{
   ...
   // echo json_encode(array("statusCode"=>201, "errorMessage"=>"User information not found in the database. Please check and try again."));
	...
	$value = dataVerification_sa($national_id,$dob,$firstname,$lastname,$identitypassbaseurl."/api/v2/biometrics/merchant/data/verification/sa/national_id", $identitypassapikey);		  
...
// echo $value;
	  ...
	  if($response_code == "00")
	  {
		  ...
		  $rlastname = $resp['data']['data']['surnameFirst'];
		  $rfirstname = $resp['data']['data']['surname_first'];
		  $rgender = $resp['data']['data']['gender'];
		  $rbirthdate = $resp['data']['data']['dob'];
		 $rphoto = $resp['data']['data']['passport'];
		 $detail = $resp['data']['data']['formofaddress'];
...
`middlename` = '$rmiddlename', `gender` = '$rgender', `dob` = '$rbirthdate', `passport` = '$passport' ,`id_number` = '$national_id',`varify_type` = 'National ID' ,`nationality` = 'SA' WHERE `account_number` = '$account_number'";
 ...
 $process = true;
	if ($process) {
		  ...
		  $rlastname = $resp['data']['data']['surnameFirst'];
		  $rmiddlename = $resp['data']['data']['completename'];
		  $rfirstname = $resp['data']['data']['surname_first'];
		  $rgender = $resp['data']['data']['gender'];
		  $rbirthdate = $resp['data']['data']['dob'];
		 $rphoto = $resp['data']['data']['passport'];
