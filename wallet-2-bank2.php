...
curl_setopt($curl, CURLOPT_HTTPHEADER, $headers);
 	...
 	$naration = "V2B transfer from ".$username."(".$amount.") to ".$beneficiaryName	;
 ...
 */
	  ...
	  $sql = 	mysqli_query($conn,"INSERT INTO `transaction_information`(`code`,   `truck_owner`,  `amount`, `commissiononfee`,  `registeredby`, `created_date`, `message`, `status`, `customer` ,  `owners_share`, `means`, `refs`, `products`, `narration` , `channel`,  `id_number`, `wallet_number` ) VALUES('$code','$sent_to_wynkid','$amount','$fee','$emailing', '$datetime','$allmessages','1', '$beneficiaryName', '0','wallet','$code','w2b','$naration','$naration','$to_wallet','$from_wallet') ")or die("ERROR OCCURED: ".mysqli_error($conn)); 			
...
echo json_encode(array("statusCode"=>200, "message"=>$message. ". Payment Successfull", "code"=>$ref, "amount"=>number_format($amount,2) )); 
