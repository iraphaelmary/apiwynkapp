...
$query1 =  "SELECT  `id`, `message`, `title`, `registeredby`, `sent_to`, `status`, `read_status`, `created_at`, `code`, `truck_id` FROM `notification` WHERE   `sent_to` = '$username' AND 	 `read_status`  = '1'AND `trip`  = '0' ORDER BY `id` DESC LIMIT 1";
  ...
  $extract_distance1 = mysqli_query($conn, $query1) or die(mysqli_error($conn));
 	 ...
 	 while ($row_distance1=mysqli_fetch_row($extract_distance1))
    {
        ...
        echo json_encode(array("statusCode"=>200,"title"=>$title, "code"=>$code, "ride"=>$ride, "status"=>$status, "id"=>$id));
