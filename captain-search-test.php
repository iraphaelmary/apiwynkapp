 ...
 /*  
{
      ...
      'wynkid': "WYNK39553840",
      'loading_long': 7.00433,
      'loading_lat': 9.08833,
      'destination_long': 8.00231,
      'destination_lat': 6.09341,
      'pickup': "Iyana Isolo",
      'destination' : "TBS",
      'stops': [{"stop_lat":9.076439100000002,"stop_lng":7.425385499999998,"stop_desc":"Jabi Lake Mall, Bala Sokoto Way, Abuja, Nigeria"},{"stop_lat":9.076439100000002,"stop_lng":7.425385499999998,"stop_desc":"Jabi Lake Mall, Bala Sokoto Way, Abuja, Nigeria"},{"stop_lat":9.076439100000002,"stop_lng":7.425385499999998,"stop_desc":"Jabi Lake Mall, Bala Sokoto Way, Abuja, Nigeria"}]
}*/
...
$username = "WYNK23378711";
    $emailing = "WYNK23378711"; 
   $mode = "cash"; 
  $pickup = "Iyana Isolo"; 
     $destination = "TBS"; 
     $stops = '{"stops": [{"stop_lat":9.076439100000002,"stop_lng":7.425385499999998,"stop_desc":"Jabi Lake Mall, Bala Sokoto Way, Abuja, Nigeria"},{"stop_lat":9.076439100000002,"stop_lng":7.425385499999998,"stop_desc":"Jabi Lake Mall, Bala Sokoto Way, Abuja, Nigeria"},{"stop_lat":9.076439100000002,"stop_lng":7.425385499999998,"stop_desc":"Jabi Lake Mall, Bala Sokoto Way, Abuja, Nigeria"}]}'; 
     
     /*
     payment_mode:cash,
wynkid:WYNK87627590,loading_long:5.3285505,loading_lat:7.7979306,destination_long:5.51449300000001,destination_lat:7.


wynkid:WYNK87627590,loading_long:5.4940607 ,loading_lat: 7.7942905,destination_long: 5.514493000000001,destination_lat:7.798266099999998,payment_mode: wallet,
 ...
 if(!empty($start))
{
      ...
      $sql = mysqli_query($conn,"INSERT INTO `search_result`(`code`, `loading`, `destination`, `pick_up_date`, `product`, `truck_type`, `truck_capacity`, `status`, `created_date`, `registeredby`, `longi1`, `lati1`, `longi2`, `lati2`, `distance`, `truck_id`, `amount`, `truck_owner`, `minutes`, `loadingcity`, `destinationcity`, `loadingcountry`,`destinationcountry`, `loadingregion`, `destinationregion`, `road_maintenance_charge`) VALUES('$code','$start','$stop', '$datetime','-','$truck_brand','-','2','$datetime','$user','$long1','$lat1','$long2','$lat2','$distance2','$truck_id', '$estimated', '$account_number', '$min_value2', '$loading_state1', '$loading_state2', '$loading_county1', '$loading_county2', '$loading_region', '$detination_region2', '$road_maintenance_charge')") or die("ERROR OCCURED: ".mysqli_error($conn)); 
 	 ...
 	 while ($row_distance12=mysqli_fetch_row($extract_distance12))
    {
         ...
         */
         ...
         if(empty($t_id) or $t_id == "")
         {
			...
			$data = json_decode($stops, true);

if (is_array($data) && isset($data['stops'])) {
    foreach ($data['stops'] as $stop) {
       // echo "Latitude: " . $stop['stop_lat'] . ", Longitude: " . $stop['stop_lng'] . ", Description: " . $stop['stop_desc'] . "\n";
		
		
		$stop_lati = $stop['stop_lat'];
		$stop_longi = $stop['stop_lng'];
...
$sql = 	mysqli_query($conn,"INSERT INTO `ride_stops`(`stop_lat`, `stop_long`,`stop_desc`, `status`, `created_date`, `registeredby`, `ride_code` ) VALUES('$stop_lati','$stop_longi','$stop_desc','1','$datetime', '$emailing', '$code') ")or die("ERROR OCCURED: ".mysqli_error($conn)); 	
		...
		//SELECT `id`, `stop_lat`, `stop_long`, `status`, `created_date`, `registeredby`, `ride_code` FROM `ride_stops` WHERE 1
