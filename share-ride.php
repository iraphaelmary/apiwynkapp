...
$username = $json['wynkid'];
  ...
  $original_destination_long = $myName->showName($conn, "SELECT `longi2` FROM `search_result` WHERE `code` = '$ride_code'");
  $original_destination_lati = $myName->showName($conn, "SELECT `lati2` FROM `search_result` WHERE `code` = '$ride_code'");


$first_route = $stop_lat.",".$stop_long; 
$second_route = $original_destination_lati.",".$original_destination_long; 
...
$json = json_decode($response, true);
 ...
 $account_number = $myName->showName($conn, "SELECT `longi2` FROM `truck_owner` WHERE `code` = '$ride_code'");
 $truck_plate_number = $myName->showName($conn, "SELECT `truck_plate_number` FROM `online_status` WHERE `account_number` = '$account_number'");
						   
	 $lati = $myName->showName($conn, "SELECT  `lati` FROM  `online_status` WHERE  `account_number` = '$account_number'");				  	 
	 $longi = $myName->showName($conn, "SELECT  `long` FROM  `online_status` WHERE  `account_number` = '$account_number'");				  	 
 ...
 $minutes_cal2= trim(substr($dist['value']/60,0,-2));
    	 ...
    	 $distance_o = round($distance_real, 2);
          ...
          $cityRetuning2 =  returnCity($lati, $longi);
 ...
 if(!empty($start))
{
      ...
      $sql = mysqli_query($conn,"INSERT INTO `search_result`(`code`, `loading`, `destination`, `pick_up_date`, `product`, `truck_type`, `truck_capacity`, `status`, `created_date`, `registeredby`, `longi1`, `lati1`, `longi2`, `lati2`, `distance`, `truck_id`, `amount`, `truck_owner`, `minutes`, `loadingcity`, `destinationcity`, `loadingcountry`,`destinationcountry`, `loadingregion`, `destinationregion`, `road_maintenance_charge`, `stop`) VALUES('$code','$start','$stop', '$datetime','-','$truck_brand','-','2','$datetime','$user','$long1','$lat1','$long2','$lat2','$distance2','$truck_id', '$estimated', '$account_number', '$min_value2', '$loading_state1', '$loading_state2', '$loading_county1', '$loading_county2', '$loading_region', '$detination_region2', '$road_maintenance_charge', '$stops.$stops2')") or die("ERROR OCCURED: ".mysqli_error($conn)); 
...
{ 
    echo json_encode(array("statusCode"=>201, "errorMessage"=>"Wynk: Your destination is not on the same route with the ride. Thank you."));    
...
{
    $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$lat1.",".$long1."&destinations=".$lat2.",".$long2."&mode=driving&language=en-EN&departure_time=now&sensor=false&key=AIzaSyCckE5OPITrw2kfYM6PWO6uuovAlqQqvTE";
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_PROXYPORT, 3128);
