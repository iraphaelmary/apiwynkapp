...
mysqli_query($conn, $sqlnot1) or die(mysqli_error($conn));
 ...
 $device_token=$myName->showName($conn, "SELECT `device_token` FROM `device_token` WHERE `account_number` = '$customer_account'"); 	
			...
			if(!empty($device_token))
			{
				...
				$fcmSend->sendFCM($device_token, "Trip Accepted",$message, "2", $statusParam);
			}			
...
mysqli_query($conn, $sqlnot) or die(mysqli_error($conn));
 ...
 $device_token=$myName->showName($conn, "SELECT `device_token` FROM `device_token` WHERE `account_number` = '$customer_account'"); 	
			...
			if(!empty($device_token))
			{
				...
				$fcmSend->sendFCM($device_token, "Wynk Trip Info",$message, "2", $statusParam);
			}			
...
mysqli_query($conn, $sqlnot) or die(mysqli_error($conn));   
 ...
 $dist2 = GetDrivingDistance($start_lat, $currentLat,$start_long,$currentLong, $googleapi);
 ...
 $estimated = $total_price_per_km + ($truck_capacity_charge * $minutes) + $base_fare + $road_maintenance_charge;
      ...
      $total_amount = number_format($estimated + $convinience_fee);
         $total =  $estimated + $convinience_fee ;
                                        ...
                                        //stop 
 $sqlnot ="UPDATE `trip_record` SET  `base_price` = '$base_fare', `distance` = '$total_price_per_km', `time` = '$total_price_pertime', `road_maintenance` = '$road_maintenance_charge', `trip_amount` = '$total',  `end_longi` = '$currentLong', `end_lati` = '$currentLat' , `convinience_fee` = '$convinience_fee'  WHERE `code` = '$id'";
mysqli_query($conn, $sqlnot) or die(mysqli_error($conn));   
  ...
  $pick_lat = $myName->showName($conn, "SELECT  `lati1` FROM  `search_result` WHERE  (`code` = '$id')");                                
     ...
     echo json_encode(array("statusCode"=>200, "errorMessage"=>"-".$statusParam." Successfully. Thank You.", "dest_long"=>$dest_long, "dest_lat"=>$dest_lat, "pick_long"=>$pick_long, "pick_lat"=>$pick_lat, "base_fare"=>$base_fare, "time"=>$total_price_pertime, "road_maintenance"=>$road_maintenance_charge, "convinience_fee"=>$convinience_fee, "total"=>$total, "trip_otp"=>$otp));
...
function returnLocation($lat, $long,$googleapi) {
    $location = "";
...
{
    $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$lat1.",".$long1."&destinations=".$lat2.",".$long2."&mode=driving&language=en-EN&departure_time=now&key=".$googleapi;
    $ch = curl_init();
