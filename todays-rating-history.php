...
// $query =  "SELECT `id`, `code`, `loading`, `destination`, `pick_up_date`,  `created_date`, `registeredby`, `amount` FROM  `search_result` WHERE (`truck_owner` = '$username' AND `status` = '1' AND `created_date` LIKE '%".$today."%')";  
 ...
 $extract_distance = mysqli_query($conn, $query) or die(mysqli_error($conn));
 	 ...
 	 while ($row_distance=mysqli_fetch_row($extract_distance))
    {
		 ...
		 //SELECT `id`, `captain`, `patron`, `review`, `ratings_score`, `created_date`, `status`, `rideid`, `registeredby`, `rated` FROM `ratings` WHERE `captain`
		 
 $review = $myName->showName($conn, "SELECT `review` FROM `ratings` WHERE `rated` = '$username' AND `rideid` = '$code'"); 
 $ratings_score = $myName->showName($conn, "SELECT `ratings_score` FROM `ratings` WHERE `rated` = '$username' AND `rideid` = '$code'"); 
 
		 
		 
		 if(!empty($review) or !empty($ratings_score))
		 {
