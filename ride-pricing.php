...
//$user = "Annonymous";
 ...
 z.location,
  ...
  WHERE  ((z.status = '1' or z.status = '9') AND (z.created_date LIKE '%".$today."%') AND z.usertype = 2 ) AND z.lati
 ...
 $minutes_cal2= trim(substr($dist2['value']/60,0,-2));
    ...
    $detination_region2 = $cityRetuning2['region'];
      //  $extract_user = mysqli_query($conn, "SELECT * FROM `road_maintenance_charge` WHERE  `state` = '$state' AND `country` = '$country'") or die(mysqli_error($conn));
         ...
         if( $loading_region ==  $loading_region2 and $loading_county2 == $loading_county1)
         {
...
function get_coordinates($address)
{
    ...
    $url = "https://maps.google.com/maps/api/geocode/json?address=$address&sensor=false&region=Poland&key=AIzaSyBZHrS0RwVvcqxuS_9y5trTV9VOdpD-N_8";
    $ch = curl_init();
...
{
    $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$lat1.",".$long1."&destinations=".$lat2.",".$long2."&mode=driving&language=en-EN&departure_time=now&key=AIzaSyBZHrS0RwVvcqxuS_9y5trTV9VOdpD-N_8";
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_PROXYPORT, 3128);
...
{
     $url = "https://maps.googleapis.com/maps/api/geocode/json?latlng=$lat,$long&key=AIzaSyBZHrS0RwVvcqxuS_9y5trTV9VOdpD-N_8";	
